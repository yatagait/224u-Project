...
class AmazonReview(Dataset):
    def __init__(self, args, mode, max_len=128, bert_model='albert-base-v1'):
        ...
        if self.mode in ['train', 'indomain-val']:
            if args.all_data:
                data_path = '{}/AmazonReview/amazon_{}.pkl'.format(args.datadir, self.mode)
            else:
                data_path = '{}/AmazonReview/amazon_500_{}.pkl'.format(args.datadir, self.mode)
            self.data = pickle.load(open(data_path, 'rb'))
        else:
            if args.all_data:
                self.data = pickle.load(open('{}/AmazonReview/amazon.pkl'.format(args.datadir), 'rb'))
            else:
                self.data = pickle.load(open('{}/AmazonReview/amazon_500.pkl'.format(args.datadir), 'rb'))
    ...
    def __getitem__(self, index):
        inputa=[]
        # outputa = np.zeros([self.args.meta_batch_size, self.k_shot * 2, 5])
        ...
        for meta_batch_id in range(self.args.meta_batch_size):
            ...
            inputa.append([token_ids_a, attn_masks_a, token_type_ids_a])
            inputb.append([token_ids_b, attn_masks_b, token_type_ids_b])
    ...
    def __init__(self, args, mode, max_len=128, bert_model='albert-base-v1'):
        ...
        if self.mode == 'train':
            ...
            if self.args.ratio<1.0:
                sel_class = int(self.args.ratio*len(self.classes_idx))
                self.classes_idx = self.classes_idx[:sel_class]
        elif self.mode =='val':
            self.classes_idx = np.array([17, 13, 18,  2, 40, 39])
        elif self.mode == 'test':
            self.classes_idx = np.array([32, 11, 23, 19, 10, 26, 12, 31, 21, 38])

        self.data = pickle.load(open('/iris/u/yatagait/Data/HuffPost/huffpost.pkl', 'rb'))
